...
# SCENIC 编码场景应该这样运行： scenic scripts.scenic --simulate
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scenario_cfg', nargs='*', type=str, default=['generate_traffic.yaml'])
    parser.add_argument('--simulator_type', nargs='*', type=str, default='CARLA')
    parser.add_argument('--continue_agent_training', '-cat', type=str2bool, required=True, default=False)
    parser.add_argument('--continue_scenario_training', '-cst', type=str2bool, required=True, default=False)
    parser.add_argument('--realtime_eval', '-rte', type=str, default='TTC', choices=['None', 'TTC']) # caixuan, 确定是否进行在线评估（TTC、势场法等）
    parser.add_argument('--process_record',  type=str2bool, required=True, default=False) # 选择是否记录过程数据_caixuan
    parser.add_argument('--save_video', type=str2bool, required=True, default=True)
