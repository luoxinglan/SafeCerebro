...
async def capture_screen(window_id):
    while True:
        try:
            ...
            # 使用 import 抓取窗口内容
            output_file_png = "/tmp/window_capture.png"
            command_import = ["import", "-window", f"{window_id}", output_file_png, "-silent"]
            print(f"执行命令: {' '.join(command_import)}")
            subprocess.run(command_import, check=True)
...
def test_import_command(window_name):
    ...
    output_file_png = "/tmp/test_capture.png"
    command_import = ["import", "-window", f"{window_id}", output_file_png, "-silent"]
    print(f"测试命令: {' '.join(command_import)}")

    try:
        subprocess.run(command_import, check=True)
        print(f"成功捕获窗口到 {output_file_png}")
