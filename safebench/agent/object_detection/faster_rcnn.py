...
class FasterRCNNAgent(BasePolicy):
    def __init__(self, config, logger, train_mode='none') -> None:
        ...
        # self.conf_thres = 0.3
        self.model.names = names_coco_paper
    ...
    def get_action(self, obs, infos, deterministic=False):
        ...
        for i in range(n_envs):
            ...
            pred = self.model(image)[0] #这个pred和yolo的不一样！
            # 将预测结果转换为带有标注框的图像
            annotated_image = self.annotate_image(image, pred)
            img_annot_list.append(annotated_image)
        ...
        return [{'ego_action': np.array([0.4, 0.0]), 'od_result': pred_list[i], 'annotated_image': img_annot_list[i]} for i in range(n_envs)]
    ...
    def annotate_image(self, image, outputs):
        """
        在图像上绘制 Faster R-CNN 模型的预测结果
        Args:
            image: 原始图像,NumPy数组格式 [H, W, C]
            outputs: Faster R-CNN 模型的输出字典
        Returns:
            annotated_image: 带有标注框的图像,NumPy数组格式 [H, W, C]
        """
        ...
        for box, label, score in zip(boxes, labels, scores):
            if  score > 0.7:
                ...
                cv2.rectangle(image, p1, p2, color, thickness=linewidth, lineType=cv2.LINE_AA)

                if label:
                    label_ = str(self.model.names[max(int(label)-1,0)]) + ' {:.2f}'.format(score)
                    tf = max(linewidth - 1, 1)  # font thickness
                    w, h = cv2.getTextSize(label_, 0, fontScale=linewidth / 3, thickness=tf)[0]  # text width, height
                    outside = p1[1] - h >= 3
                    p2 = p1[0] + w, p1[1] - h - 3 if outside else p1[1] + h + 3
                    cv2.rectangle(image, p1, p2, color, -1, cv2.LINE_AA)  # filled
                    cv2.putText(image,
                                label_, (p1[0], p1[1] - 2 if outside else p1[1] + h + 2),
                                0,
                                linewidth / 3,
                                txt_color,
                                thickness=tf,
