...
class SignalizedJunctionRightTurn(BasicScenario):
    ...
    def convert_actions(self, actions):
        ...
        v_scale = 10
        d_scale = 5
        x_mean = v_mean = dist_mean = 0

        x = actions[0] * x_scale + x_mean
        v = actions[2] * v_scale + v_mean
        dist = actions[1] * d_scale + dist_mean
        return [x, dist, v]
    ...
    def create_behavior(self, scenario_init_action):
        actions = self.convert_actions(scenario_init_action)
        self.x, delta_dist, delta_v = actions  
        self._target_vel = 1 + delta_v
        self.trigger_distance_threshold = 50 + delta_dist
...
class NoSignalJunctionCrossingRoute(BasicScenario):
    ...
    def convert_actions(self, actions):
        ...
        delta_v_scale = 3
        d_scale = 10
        x_mean = y_mean =  dist_mean = 0
        delta_v_mean = 9
        
        if len(actions) == 2:
            x = 0 * x_scale + x_mean
            y = 0 * y_scale + y_mean
            delta_v = actions[0] * delta_v_scale + delta_v_mean
            dist = actions[1] * d_scale + dist_mean
        elif len(actions) == 4:
            x = actions[0] * x_scale + x_mean
            y = actions[1] * y_scale + y_mean
            delta_v = actions[3] * delta_v_scale + delta_v_mean
            dist = actions[2] * d_scale + dist_mean
        return [x, y, delta_v, dist]
