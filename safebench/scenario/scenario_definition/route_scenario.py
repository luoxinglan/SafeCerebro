...
class RouteScenario():
    ...
    def _create_criteria(self):
        ...
        criteria['collision'] = CollisionTest(actor=self.ego_vehicle, terminate_on_failure=True) # caixuan
    ...
    def _get_actor_state(actor):
        ...
        # return [actor_x, actor_y, actor_yaw, yaw[0], yaw[1], velocity.x, velocity.y, acc.x, acc.y] # caixuan，状态信息
        return [actor_x, actor_y, velocity.x, velocity.y]
    ...
    def update_info(self):
        ...
        for s_i in self.list_scenarios:
            for a_i in s_i.other_actors:
                ...
                # actor_state = [a - b for a, b in zip(actor_state, ego_state)] # caixuan, npc坐标全部变更为相对位置
