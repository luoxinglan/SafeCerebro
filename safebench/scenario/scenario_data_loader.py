...
class ScenarioDataLoader:
    ...
    def sampler(self):
        ...
        # select scenarios_caixuan，scenario采样的所有类型必须保证是一致的，比如交通车的数量必须一致，否则报错！
        # selected_idx = np.random.choice(self.scenario_idx, size=sample_num, replace=False).tolist()
        selected_idx = self._select_non_overlap_idx(self.scenario_idx, sample_num) 
        # selected_idx = np.random.choice([4,16,20,24,28],size=sample_num,replace=False).tolist() #caixuan
        selected_scenario = []
        ...
        return selected_scenario, len(selected_scenario) #caixuan
...
class ScenicDataLoader:
    ...
    def scenicToCarlaLocation(self, points):
        waypoints = []
        for point in points:
            location = cerebro.Location.get_Location(point[0], -point[1], 0.0)
