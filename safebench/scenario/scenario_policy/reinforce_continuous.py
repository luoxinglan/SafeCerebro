...
class IndependantModel(nn.Module):
    ...
    def forward(self, x, determinstic):
        ...
        # p(b|a,s)
        s = torch.cat((s, mu_a), dim=1) if determinstic else torch.cat((s, action_a), dim=1) 
        normal_b = self.fc_action_b(s)
        action_b, mu_b, sigma_b = self.sample_action(normal_b, self.b_os)

        # p(c|a,b,s)
        s = torch.cat((s, mu_b), dim=1) if determinstic else torch.cat((s, action_b), dim=1) 
        normal_c = self.fc_action_c(s)
        action_c, mu_c, sigma_c = self.sample_action(normal_c, self.c_os)

        # p(d|a,b,c,s)
        s = torch.cat((s, mu_c), dim=1) if determinstic else torch.cat((s, action_c), dim=1) 
...
class REINFORCE(BasePolicy):
    ...
    def __init__(self, scenario_config, logger):
        ...
        self.entropy_weight = 1.0e-4
        self.itr_per_train = scenario_config['itr_per_train']

        self.standard_action_dim = True
        self.model = CUDA(IndependantModel(self.num_waypoint))
        self.optimizer = optim.Adam(self.model.parameters(), lr=self.lr)

    def train(self, replay_buffer):
        if replay_buffer.init_buffer_len < 0: #每4代迭代一次
            return
        ...
        episode_reward = + episode_reward # objective is to minimize the reward: greater reward of ego, greater loss of scenario, caixuan
        ...
        self.optimizer.step()
        self.logger.log('>> Training loss: {:.4f}'.format(loss.item()),color="blue")
                ...
                for name in files:
                    if fnmatch(name, "*torch"):
                        ...
                        if cur_episode > episode:
                            episode = cur_episode
        self.model = CUDA(IndependantModel(self.num_waypoint, self.standard_action_dim))
        self.optimizer = optim.Adam(self.model.parameters(), lr=self.lr)
        # model_filename = os.path.join(self.model_path, str(scenario_id), model_file)
        model_filename = os.path.join(self.model_path, f'model.LC.{self.model_id}.{episode:04}.torch')
    ...
    def save_model(self, epoch):
        ...
        model_filename = os.path.join(self.model_path, f'model.LC.{self.model_id}.{epoch:04}.torch')
