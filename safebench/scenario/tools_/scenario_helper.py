...
def get_crossing_point(actor):
    ...
    crossing = cerebro.Location.get_Location(x=wp_cross.transform.location.x, y=wp_cross.transform.location.y, z=wp_cross.transform.location.z)
    return crossing
...
def get_geometric_linear_intersection(ego_actor, other_actor):
    ...
    intersection = cerebro.Location.get_Location(x=x / z, y=y / z, z=0)
    return intersection
...
def generate_target_waypoint_list_multilane(
    ...
    while lane_changes_done < lane_changes:
        ...
        if not side_wp or side_wp.lane_type != cerebro.LaneType.Driving:
            return None, None
...
def choose_at_junction(current_waypoint, next_choices, direction=0):
    ...
    projected_location = current_location + cerebro.Location.get_Location(
...
def get_intersection(ego_actor, other_actor):
    ...
    while distance <= max_dist:
        ...
        if len(waypoint_choice) > 1:
            max_dot = -1 * float('inf')
            loc_projection = current_location + cerebro.Location.get_Location(
...
def detect_lane_obstacle(actor, extension_factor=3, margin=1.02):
    ...
    actor_location = actor_location + cerebro.Location.get_location(actor_vector[0], actor_vector[1])
...
def get_junction_topology(junction):
    ...
    entry_wps = []
    exit_wps = []
    for entry_wp, exit_wp in junction.get_waypoints(cerebro.LaneType.Driving):
