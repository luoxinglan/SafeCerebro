...
def convert_json_to_transform(actor_dict):
    ...
    """
    return cerebro.Transform.get_Transform(
        location=cerebro.Location.get_Location(x=float(actor_dict['x']), y=float(actor_dict['y']), z=float(actor_dict['z'])),
        rotation=cerebro.Rotation.get_Rotation(roll=0.0, pitch=0.0, yaw=float(actor_dict['yaw']))
    )
...
class ActorConfigurationData(object):
    ...
    def parse_from_node(node, rolename):
        ...
        transform = cerebro.Transform.get_Transform(cerebro.Location.get_Location(x=pos_x, y=pos_y, z=pos_z), cerebro.Rotation.get_Rotation(yaw=yaw))
