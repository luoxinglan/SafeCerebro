...
class RouteParser(object):
        ...
        for route in tree.iter("route"):
            ...
            for waypoint in route.iter('waypoint'):
                if len(waypoint_list) == 0:
                    ...
                    initial_pose = cerebro.Transform.get_Transform(cerebro.Location.get_Location(x, y, z), \
                                                                   cerebro.Rotation.get_Rotation(roll=roll, pitch=pitch, yaw=yaw))
                    initial_pose = cerebro.Transform.get_Transform(cerebro.Location.get_Location(x, y, z), \
                                                                   cerebro.Rotation.get_Rotation(roll=roll, pitch=pitch, yaw=yaw))
                    new_config.initial_transform = initial_pose
                    new_config.initial_pose = initial_pose
                waypoint_list.append(cerebro.Location.get_Location(x=float(waypoint.attrib['x']), y=float(waypoint.attrib['y']), z=float(waypoint.attrib['z'])))
    ...
    def parse_weather(route):
        ...
        if route_weather is None:
            weather = cerebro.WeatherParameters.get_WeatherParameters(sun_altitude_angle=70)
        else:
            weather = cerebro.WeatherParameters.get_WeatherParameters()
