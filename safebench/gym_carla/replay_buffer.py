...
class RouteReplayBuffer:
    ...
    def sample(self, batch_size, shffule=True):
        ...
        if shffule == True:
            ...
            # 顺序采样
            sample_index = np.arange(batch_size) if batch_size <= len(prepared_rewards) else np.arange(len(prepared_rewards))[:batch_size]
