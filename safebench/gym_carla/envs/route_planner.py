...
class RoutePlanner():
    def __init__(self, vehicle, buffer_size, init_waypoints):
        ...
        if len(init_waypoints) == 0:
            project_waypoint =  self._map.get_waypoint(self._vehicle.get_location(), project_to_road=True, lane_type=cerebro.LaneType.Driving)
        ...
        if self._target_waypoint is not None:
            if self._target_waypoint.is_intersection:
                ...
                if sel_traffic_light is not None:
                    ...
                    if self._last_traffic_light.state == cerebro.libcarla.TrafficLightState.Red:
                        return True
                else:
                    self._last_traffic_light = None
