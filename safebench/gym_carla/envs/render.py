...
class Util(object):
    ...
    def get_bounding_box(actor):
        ...
        corners = [
            cerebro.Location.get_Location(x=-bb.x, y=-bb.y),
            cerebro.Location.get_Location(x=bb.x, y=-bb.y),
            cerebro.Location.get_Location(x=bb.x, y=bb.y),
            cerebro.Location.get_Location(x=-bb.x, y=bb.y),
            cerebro.Location.get_Location(x=-bb.x, y=-bb.y)
...
class MapImage(object):
    ...
    def draw_road_map(self, map_surface, carla_world, carla_map, world_to_pixel, world_to_pixel_width):
        ...
        def lane_marking_color_to_tango(lane_marking_color):
            ...
            if lane_marking_color == cerebro.LaneMarkingColor.White:
                tango_color = COLOR_ALUMINIUM_2

            elif lane_marking_color == cerebro.LaneMarkingColor.Blue:
                tango_color = COLOR_SKY_BLUE_0

            elif lane_marking_color == cerebro.LaneMarkingColor.Green:
                tango_color = COLOR_CHAMELEON_0

            elif lane_marking_color == cerebro.LaneMarkingColor.Red:
                tango_color = COLOR_SCARLET_RED_0

            elif lane_marking_color == cerebro.LaneMarkingColor.Yellow:
        ...
        def get_lane_markings(lane_marking_type, lane_marking_color, waypoints, sign):
            ...
            if lane_marking_type == cerebro.LaneMarkingType.Broken or (lane_marking_type == cerebro.LaneMarkingType.Solid):
                return [(lane_marking_type, lane_marking_color, marking_1)]
            else:
                marking_2 = [world_to_pixel(lateral_shift(w.transform, sign * (w.lane_width * 0.5 + margin * 2))) for w in waypoints]
                if lane_marking_type == cerebro.LaneMarkingType.SolidBroken:
                    return [(cerebro.LaneMarkingType.Broken, lane_marking_color, marking_1), (cerebro.LaneMarkingType.Solid, lane_marking_color, marking_2)]
                elif lane_marking_type == cerebro.LaneMarkingType.BrokenSolid:
                    return [(cerebro.LaneMarkingType.Solid, lane_marking_color, marking_1), (cerebro.LaneMarkingType.Broken, lane_marking_color, marking_2)]
                elif lane_marking_type == cerebro.LaneMarkingType.BrokenBroken:
                    return [(cerebro.LaneMarkingType.Broken, lane_marking_color, marking_1), (cerebro.LaneMarkingType.Broken, lane_marking_color, marking_2)]

                elif lane_marking_type == cerebro.LaneMarkingType.SolidSolid:
                    return [(cerebro.LaneMarkingType.Solid, lane_marking_color, marking_1), (cerebro.LaneMarkingType.Solid, lane_marking_color, marking_2)]

            return [(cerebro.LaneMarkingType.NONE, cerebro.LaneMarkingColor.Other, [])]
        ...
        def draw_lane_marking_single_side(surface, waypoints, sign):
            ...
            marking_type = cerebro.LaneMarkingType.NONE
            previous_marking_type = cerebro.LaneMarkingType.NONE

            marking_color = cerebro.LaneMarkingColor.Other
            previous_marking_color = cerebro.LaneMarkingColor.Other

            markings_list = []
            temp_waypoints = []
            current_lane_marking = cerebro.LaneMarkingType.NONE
            ...
            for markings in markings_list:
                if markings[0] == cerebro.LaneMarkingType.Solid:
                    draw_solid_line(surface, markings[1], False, markings[2], 2)
                elif markings[0] == cerebro.LaneMarkingType.Broken:
        ...
        def draw_traffic_signs(surface, font_surface, actor, color=COLOR_ALUMINIUM_2, trigger_color=COLOR_PLUM_0):
            ...
            forward_vector = cerebro.Location.get_from_forward_vector(waypoint.transform.get_forward_vector())
            left_vector = cerebro.Location.get_Location(-forward_vector.y, forward_vector.x, forward_vector.z) * waypoint.lane_width / 2 * 0.7
        ...
        def draw_topology(carla_topology, index):
            ...
            for waypoint in topology:
                ...
                for w in waypoints:
                    l = w.get_left_lane()
                    while l and l.lane_type != cerebro.LaneType.Driving:

                        if l.lane_type == cerebro.LaneType.Shoulder:
                            shoulder[0].append(l)

                        if l.lane_type == cerebro.LaneType.Parking:
                            parking[0].append(l)

                        if l.lane_type == cerebro.LaneType.Sidewalk:
                            sidewalk[0].append(l)

                        l = l.get_left_lane()

                    r = w.get_right_lane()
                    while r and r.lane_type != cerebro.LaneType.Driving:

                        if r.lane_type == cerebro.LaneType.Shoulder:
                            shoulder[1].append(r)

                        if r.lane_type == cerebro.LaneType.Parking:
                            parking[1].append(r)

                        if r.lane_type == cerebro.LaneType.Sidewalk:
                            sidewalk[1].append(r)
    ...
    def _render_hist_actors(self, surface, actor_polygons, actor_type, world_to_pixel, num):
        ...
        for i in range(max(0,lp-num), lp):
            for ID, poly in actor_polygons[i].items():
                corners = []
                for p in poly:
                    corners.append(cerebro.Location.get_Location(x=p[0], y=p[1]))
                corners.append(cerebro.Location.get_Location(x=poly[0][0], y=poly[0][1]))
