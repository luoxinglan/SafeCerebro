...
class CarlaEnv(gym.Env):
    ...
    def _create_sensors(self):
        ...
        if self.scenario_category != 'perception':
            # lidar sensor
            self.lidar_trans = cerebro.Transform.get_Transform(cerebro.Location.get_Location(x=0.0, z=self.lidar_height))
        ...
        self.camera_trans = cerebro.Transform.get_Transform(cerebro.Location.get_Location(x=0.8, z=1.7))
    ...
    def _parse_route(self, config):
        ...
        for node in route:
            loc = node[0].location
            waypoint = carla_map.get_waypoint(loc, project_to_road=True, lane_type=cerebro.LaneType.Driving)
    ...
    def _attach_sensor(self):
        ...
        self.collision_sensor = self.world.spawn_actor(self.collision_bp, cerebro.Transform.get_Transform(), attach_to=self.ego_vehicle)
        ...
        if self.world:
            ...
            if snapshot:
                ...
                if not self.auto_ego:
                    ...
                    # apply control
                    act = cerebro.VehicleControl.set_control(throttle=float(throttle), steer=float(steer), brake=float(brake))
    ...
    def _init_traffic_light(self):
        ...
        for actor in actor_list:
            if isinstance(actor, cerebro.TrafficLight.get_TrafficLight):
    ...
    def _get_reward(self):
        ...
        # CAIXUAN
        pos_all = self.vehicle_trajectories[0]
        x,y=[],[]
        for key,value in pos_all.items():
            x.append(value.location.x)
            y.append(value.location.y)
        total_distance = self.calculate_total_distance(x, y) #两车距离
        ...
        if self.ego_vehicle.get_transform().location.y > -70:
            reward_aux = 1
        else:
            reward_aux = 0
        
        # for key,value in self.vehicle_velocities[0].items(): # NPC，EGO
        #     vx = value.x
        #     vy = value.y
        #     break
        # v_npc = (vx**2+vy**2)**0.5
        # r_npc_slow = 0.0 if v_npc < 1 else 0

        # combine all rewards _ caixuan: r_collision from 1 to 10
        r = 100 * r_collision + 1 * reward_curr + 1 * reward_aux
        return r

    def calculate_total_distance(self, x_coords, y_coords):
        x = np.array(x_coords)
        y = np.array(y_coords)
