...
def get_realtime_eval(rte,infos):
    # caixuan
    if rte=='TTC':
        ref_x, ref_y, ref_vx, ref_vy = infos[0]['actor_info'][0]
    
        ttcs = []  # 存储每辆车相对于第一辆车的TTC
        
        for info in infos[0]['actor_info'][1:]:  # 跳过第一辆车
            x, y, v_x, v_y = info
            ...
            if rel_vx == 0 and rel_vy == 0:
                ...
                # 计算TTC
                # 使用相对位置和相对速度的点积
                numerator = rel_x * rel_x + rel_y * rel_y
                denominator = rel_x * rel_vx + rel_y * rel_vy
                
                # 如果分母大于0,说明车辆正在远离,不会碰撞
                if denominator >= 0:
                    ttc = float('inf')
                else:
