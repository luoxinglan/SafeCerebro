...
from cerebro import SIMULATOR_POLICY_LIST
...
from safebench.util.metric_util import get_route_scores, get_perception_scores, get_realtime_eval
...
class CarlaRunner:
    def __init__(self, agent_config, scenario_config):
        ...
        self.process_record = scenario_config['process_record']
        self.rte = scenario_config['realtime_eval']
        ...
        self.simulator_policy = SIMULATOR_POLICY_LIST[scenario_config['simulator_type']]
        self.simulator_policy.init_client('localhost',scenario_config['port'])
        self.client = self.simulator_policy.get_client()
    ...
    def _init_world(self, town):
        ...
        self.simulator_policy.set_client(self.client)
        self.simulator_policy.set_world(self.world)
        self.simulator_policy.set_traffic_manager_port(self.scenario_config['tm_port'])
        self.simulator_policy.set_weather('clear')
        ...
        for e_i in tqdm(range(start_episode, self.train_episode)):
            ...
            while not self.env.all_scenario_done():
                ...
                if self.process_record:
                    ego_records.append({'obs:':obs, 'ego_actions:':ego_actions})

                # apply action to env and get obs
                next_obs, rewards, dones, infos = self.env.step(ego_actions=ego_actions, scenario_actions=scenario_actions)
                if self.scenario_policy.name == 'DQN': #离散的 RL
                    replay_buffer.store([ego_actions, [self.scenario_policy.action], obs, next_obs, rewards, dones], additional_dict=infos)
                else:
                    replay_buffer.store([ego_actions, scenario_actions, obs, next_obs, rewards, dones], additional_dict=infos)
                
                buffer_one_off.append([obs,ego_actions,next_obs,rewards,dones])
            ...
            if np.sum(episode_reward)<-10: #只有发生碰撞的时候再记录，增加效率
                self.logger.add_training_results('ego_records', ego_records) #保存很费时间！,不用记得关闭caixuan
            ...
            if self.mode == 'train_agent' and self.agent_policy.type == 'onpolicy':
                ...
                self.scenario_policy.train(replay_buffer)
            elif self.mode == 'train_agent' and self.agent_policy.type == 'IRL':
                self.agent_policy.learn(buffer_one_off)
                buffer_one_off=[]
    ...
    def eval(self, data_loader):
        ...
        while len(data_loader) > 0:
            ...
            while not self.env.all_scenario_done():
                ...
                # save process record expert log
                if self.process_record:
                    expert_records.append({'obs:':obs, 'ego_actions:':ego_actions})
                ...
                # Safety - real-time evaluation, caixuan
                if self.rte == 'TTC':
                    TTC_list = get_realtime_eval(self.rte, infos)
                    for TTC_ in TTC_list:
                        text = font.render(f"TTC: {TTC_:.1f}",True,(255,255,255))
                        self.display.blit(text, (10,10))
                        pygame.display.flip()
                    if TTC_list:
                        TTC_records.append(TTC_list[0])
            ...
            if settle==False and self.scenario_config['policy_type'] == 'Generate_Traffic':
                import subprocess
                subprocess.Popen(["python", "safebench/scenario/scenario_policy/generate_traffic/generate_traffic.py"])
                time.sleep(10)
                settle = True
            ...
            if len(self.env.running_results) % 1 == 0:
                self.logger.save_eval_results()

        if self.process_record:
            expert_records = np.array(expert_records)
            np.save('./trajectory_expert.npy',expert_records)
        #save TTC
        if self.rte == 'TTC':
            self.logger.save_ttc(TTC_all)
