...
def get_realtime_eval(rte,infos):
    # caixuan
    if rte=='TTC':
        x_list, y_list, yaw_list, cos_yaw_list, sin_yaw_list, v_list = [], [], [], [], [], []
        for info in infos[0]['actor_info']:
            x, y, yaw, cos_yaw, sin_yaw, v_x, v_y, a_x, a_y = info
            x_list.append(x)
            y_list.append(y)
            cos_yaw_list.append(cos_yaw)
            sin_yaw_list.append(sin_yaw)
            v_list.append(math.sqrt(v_x**2 + v_y**2))
        
        # check if the npc is in front of the ego
        # 定义前向视场角的一半
        half_fov_angle = 5  # 单位为度
        half_fov_radians = math.radians(half_fov_angle)

        front_id = []
        relative_dis_list = [0]
        for i in range(1, len(x_list)):
            xi, yi = x_list[i], y_list[i]
            dx, dy = xi - x_list[0], yi - y_list[0]
            relative_x = dx * cos_yaw_list[0] + dy * sin_yaw_list[0]
            relative_y = -dx * sin_yaw_list[0] + dy * cos_yaw_list[0]
            relative_dis = math.sqrt(relative_x**2 + relative_y**2)
            relative_dis_list.append(relative_dis)

            # 计算相对位置向量与第一辆车正前方方向之间的夹角
            angle = math.atan2(relative_y, relative_x)
            if relative_x > 0 and abs(angle) <= half_fov_radians and relative_dis<=30:
                front_id.append(i)
    
    TTC_list = []
    for id in front_id:
        TTC = (relative_dis_list[id]-4)/(v_list[0]-v_list[id])
        TTC_list.append(TTC)
    return TTC_list
